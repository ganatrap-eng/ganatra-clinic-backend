...
const { fyRange, assetDepForFY, assetWDVAsOf } = require("../utils/depreciation");
...
// GET /api/statements/income?fy=2025-26
router.get("/income", async (req, res) => {
  ...
  const totalIncome = incomeCollection + incomeReferral;
  ...
  const depreciation = assetsR.rows.reduce((s, a) => s + assetDepForFY(a, fy).dep, 0);
  ...
  res.json({
    fy, period: { start, end },
    income: { patientCollection: incomeCollection, referral: incomeReferral, total: totalIncome },
...
router.get("/balance-sheet", async (req, res) => {
  ...
  const cumulativeIncome = (await sumUpTo("collections", "collection_date", "amount_due", end)) + (await sumUpTo("referrals", "referral_date", "amount", end));
