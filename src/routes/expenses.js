...
router.get("/", requirePermission("expenses", "view"), logAccess("expenses"), async (req, res) => {
...
router.get("/category-totals", requirePermission("expenses", "view"), logAccess("expenses"), async (req, res) => {
  const { from, to } = req.query;
  const params = [];
  let where = "";
  if (from && to) { params.push(from, to); where = "WHERE expense_date BETWEEN $1 AND $2"; }
...
router.post("/", requirePermission("expenses", "write"), logAccess("expenses"), async (req, res) => {
