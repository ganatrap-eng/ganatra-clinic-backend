...
router.get("/users", async (req, res) => {
  const r = await pool.query(
    `SELECT id, user_id, name, role, email, mobile, status, permissions, created_at
     FROM users ORDER BY (status = 'pending_approval') DESC, created_at DESC`
  );
  res.json(r.rows);
});

// PUT /api/admin/users/:id/permissions  { role, permissions, activate }
router.put("/users/:id/permissions", async (req, res) => {
  const { role, permissions, activate } = req.body;
  if (permissions) {
  ...
  const r = await pool.query(`UPDATE users SET ${fields.join(", ")} WHERE id = $${i} RETURNING id, user_id, name, role, status, permissions`, values);
