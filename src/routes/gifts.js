...
router.put("/:id", requirePermission("gifts", "edit"), logAccess("gifts"), async (req, res) => {
  const { date, repName, company, gift, doctorId, amount } = req.body;
  if (!date || !repName) return res.status(400).json({ error: "date and repName are required" });
  const r = await pool.query(
    `UPDATE gifts SET gift_date=$1, rep_name=$2, company=$3, gift_description=$4, doctor_id=$5, amount=$6
     WHERE id=$7 RETURNING *`,
    [date, repName, company || null, gift || null, doctorId || null, Number(amount) || 0, req.params.id]
  );
  if (r.rowCount === 0) return res.status(404).json({ error: "Gift entry not found" });
  res.json(r.rows[0]);
});
