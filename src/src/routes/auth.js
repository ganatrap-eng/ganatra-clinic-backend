...
router.post("/register", async (req, res) => {
  ...
  // The literal "demo" account is a testing convenience for the sample-data
  // button and bypasses admin approval on purpose — remove this branch (and
  // the "demo" button in the app) before relying on this for real patient data.
  const isDemoBootstrap = userId === "demo";
  const passwordHash = await bcrypt.hash(password, 12);

  if (isDemoBootstrap) {
    const r = await pool.query(
      `INSERT INTO users (user_id, password_hash, name, role, status, permissions)
       VALUES ($1,$2,$3,'Reception','active',$4) RETURNING *`,
      [userId, passwordHash, name || "Front Desk (Demo)", JSON.stringify(fullPermissions())]
    );
    const user = r.rows[0];
